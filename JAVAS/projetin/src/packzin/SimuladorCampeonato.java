...
public class SimuladorCampeonato {
    public static void main(String[] args) {
        ...
        int[][] tabela = new int[NUM_TEAMS][2];  // Array para armazenar pontos e saldo de gols

        for (int i = 0; i < NUM_TEAMS; i++) {
            tabela[i][0] = random.nextInt(50);  // Inicializa a pontuação aleatoriamente
        }
        ...
        for (int round = 1; round <= NUM_ROUNDS; round++) {
            ...
            for (int i = 0; i < NUM_TEAMS; i++) {
                ...
                if (resultado == 0) {
                    tabela[i][0] += POINTS_LOSS;
                } else if (resultado == 1) {
                    tabela[i][0] += POINTS_DRAW;
                } else {
                    tabela[i][0] += POINTS_WIN;
                }
            }

            // Ordena a tabela classificatória com base nos pontos
            java.util.Arrays.sort(tabela, (a, b) -> Integer.compare(b[0], a[0]));
        }
        ...
        for (int i = 0; i < NUM_TEAMS; i++) {
            ...
            System.out.println(posicao + "ª - Equipe " + (i + 1) + ": " + tabela[i][0] + " pontos");
        }
